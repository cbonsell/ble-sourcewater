...
data_means <- bind_cols(input,a2) %>% 
  pivot_longer(cols=SIM:PML, names_to = "Source") %>% 
  group_by(Lagoon, Station, DepthCat, Habitat, Season, Source, delta_18O, Sal) %>% 
  summarize(mFrac=mean(value), mDate=mean(Date_collected))
...
table2 <- data_means %>% 
  ...
  group_by(Lagoon, DepthCat, Season, Source, delta_18O) %>% 
...
ggplot(ctd, aes(date, salinity, group=instrument))+
...
mixingplot <- function(m,lagoon){
  ...
  ggplot(input[input$Lagoon==lagoon,], aes(Sal, delta_18O))+
    geom_abline(slope= mod_sim0$coefficients[[2]],intercept = mod_sim0$coefficients[[1]])+
    geom_abline(slope= mod_sim25$coefficients[[2]],intercept = mod_sim25$coefficients[[1]])+
    geom_abline(slope= mod_sim50$coefficients[[2]],intercept = mod_sim50$coefficients[[1]])+
    geom_abline(slope= mod_sim75$coefficients[[2]],intercept = mod_sim75$coefficients[[1]])+
    geom_abline(color="grey",slope= mod_MW0$coefficients[[2]],intercept = mod_MW0$coefficients[[1]])+
    geom_abline(color="grey",slope= mod_MW25$coefficients[[2]],intercept = mod_MW25$coefficients[[1]])+
    geom_abline(color="grey",slope= mod_MW50$coefficients[[2]],intercept = mod_MW50$coefficients[[1]])+
    geom_abline(color="grey",slope= mod_MW75$coefficients[[2]],intercept = mod_MW75$coefficients[[1]])+
    geom_point(aes(color=Season, shape=Habitat), size=3, alpha=.7)+
    scale_color_manual(values=Pal[c(1,10,20)])+
    theme_cowplot()+
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#plot
mixingplot(-12.6,"Elson")+
  ggtitle("Elson")

mixingplot(-17.3, "Simps")+
  ggtitle("Simpson")

mixingplot(-20.2, "Steff")+
  ggtitle("Stefansson Sound")

mixingplot(-21.4, "Jago")+
  ggtitle("Jago")
