...
input %>% filter (Habitat=="River") %>% 
  group_by(Station) %>% 
  summarize(mean_d18O=mean(delta_18O), sd=sd(delta_18O))
...
$-2*SIM + -22*MW + -3.5*PML = O$

```{r create_function, echo=FALSE, message=FALSE, warning=FALSE}

calc_source <- function(S, O){
  
  A <- matrix(data=c(1, 1, 1, 5, 0, 32.1, -2, -22, -3.5), nrow=3, ncol=3, byrow=TRUE)    
...
data_means <- bind_cols(input,a2) %>% 
  pivot_longer(cols=SIM:PML, names_to = "Source") %>% 
  group_by(Lagoon, Station, DepthCat, Habitat, Season, Source, delta_18O) %>% 
