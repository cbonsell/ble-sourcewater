...
###These lagoons exhibit large variability in salinity over the year. What causes this?

###To find out, we used salinity and 
$\delta ^{18}O$ 
###of lagoon waters sampled in April (full sea ice cover), June(sea ice break-up), and August (open water summer) to determine the relative contribution of sea ice melt (SIM), marine Polar Mixed Layer (PML), and meteoric water (MW) to these dynamic lagoons. Each of these endmembers has distinct salinity and 
$\delta ^{18}O$
###values. 

$\delta ^{18}O$
###of MW actually varies longitudinally, as seen by the pattern in freshwater inflow samples, here with stations ordered west->east:
...
where $x$ is the value for rivers associated with a particular lagoon
...
# a <- a %>% mutate(PML=case_when(PML>MW&SIM<0&MW>=0 ~ PML+SIM,
#                                 PML>SIM&MW<0&SIM>=0 ~PML+MW,
#                                 MW<0&SIM<0 ~PML+MW+SIM,
#                                 TRUE ~PML))
#                   #                  MW=case_when(MW>SIM&MW>PML))
